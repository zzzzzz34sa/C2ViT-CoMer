...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        embed_dim: int = 768,  # 图像块嵌入维度。
        depth: int = 12,  # ViT深度（块的数量）。
        num_heads: int = 12,  # 每个ViT块中的注意力头数。
        mlp_ratio: float = 4.0,  # MLP隐藏维度与嵌入维度之比。
        out_chans: int = 256,  # 输出通道数。
        qkv_bias: bool = True,  # 查询、键、值添加偏置。
        norm_layer: Type[nn.Module] = nn.LayerNorm,  # 归一化层。
        act_layer: Type[nn.Module] = nn.GELU,  # 激活层。
        use_abs_pos: bool = True,  # 是否使用绝对位置嵌入。
        use_rel_pos: bool = False,  # 是否使用相对位置嵌入。
        rel_pos_zero_init: bool = True,  # 相对位置参数零初始化。
        ...
        Args:
            ...
            mlp_ratio (float): MLP 隐藏维度与嵌入维度之比。
            qkv_bias (bool): 若为 True，为查询、键、值添加可学习的偏置。
            norm_layer (nn.Module): 归一化层。
            act_layer (nn.Module): 激活层。
            use_abs_pos (bool): 若为 True，使用绝对位置嵌入。
            use_rel_pos (bool): 若为 True，在注意力图上添加相对位置嵌入。
            rel_pos_zero_init (bool): 若为 True，零初始化相对位置参数。
