...
class ViTCoMer(TIMMVisionTransformer):
    def __init__(self, pretrain_size=224, num_heads=12, conv_inplane=64, n_points=4, deform_num_heads=6,
        ...
        self.level_embed = nn.Parameter(torch.zeros(3, embed_dim))  # 三个等级的嵌入参数
        self.spm = CNN(inplanes=conv_inplane, embed_dim=embed_dim)  # CNN 模块
        ...
        self.up = nn.ConvTranspose2d(embed_dim, embed_dim, 2, 2)  # 转置卷积层用于上采样
        self.norm1 = nn.SyncBatchNorm(embed_dim)  # 同步批归一化层
    ...
    def forward(self, x):
        ...
        if self.add_vit_feature:
            x1, x2, x3, x4 = outs
            x1 = F.interpolate(x1, scale_factor=4, mode='bilinear', align_corners=False) #插值上采样
